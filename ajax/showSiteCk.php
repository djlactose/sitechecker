<table border=1>
<tr align=\"center\"><td colspan="3" align="center"><b>Site Check Results</b></td></tr>
...
if($_GET["DateTime"]){
	...
	if($newStart>0)
	        $sql="Select * FROM SiteCk WHERE `Date` BETWEEN \"$sDateTime\" AND ADDTIME(\"$sDateTime\",\"0 5:00:00\") ORDER BY Date DESC LIMIT $newStart,31";
	else
	        $sql="Select * FROM SiteCk WHERE `Date` BETWEEN \"$sDateTime\" AND ADDTIME(\"$sDateTime\",\"0 5:01:00\") ORDER BY Date DESC LIMIT $startNum,31";
}else{
        $DateTime="NOW()";
	if($newStart>0)
	        $sql="Select * FROM SiteCk WHERE DATE_SUB(NOW(), INTERVAL 5 HOUR) <= `Date` ORDER BY Date DESC LIMIT $newStart,31";
	else
	        $sql="Select * FROM SiteCk WHERE DATE_SUB(NOW(), INTERVAL 5 HOUR) <= `Date` ORDER BY Date DESC LIMIT $startNum,30";
}
